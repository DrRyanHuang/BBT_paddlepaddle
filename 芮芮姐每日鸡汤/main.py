...
#    config = util.getConfig(filename)
    ...
    R, G, B = util.splitRGB(font_color)
    ...
    yi_file = config.get('last_line', 'yi_file')
    ji_file = config.get('last_line', 'ji_file')
    
    yi_word = util.delYiJi(yi_file)
    w, h = util.getCharacterSize(background, yi_word, font_file, textSize=font_size)
    background = util.cv2ImgAddText(background, yi_word, x1+yiji_shift-w//2, y-h//2, yiji_font_file, textColor=(R, G, B), textSize=yiji_font_size)
    
    ji_word = util.delYiJi(ji_file)
    w, h = util.getCharacterSize(background, ji_word, font_file, textSize=font_size)
    background = util.cv2ImgAddText(background, ji_word, x2+yiji_shift-w//2, y-h//2, yiji_font_file, textColor=(R, G, B), textSize=yiji_font_size)
    ...
    R, G, B = util.splitRGB(year_month_color)
...
#                                          right_content.format(idx), 
    ...
    jitang = random.choice(jitang_content.split('\n&\n'))
